...
@router.get("/reddit", dependencies=[Depends(require_perm([Role.ADMIN, Role.DEVELOPER]))])
async def scrape_reddit(
    ...
    if auto_dataset:
        if not dataset_id:
            raise HTTPException(status_code=400, detail="dataset_id is required when auto_dataset=True")
